...
export default function ProblemDetail() {
      ...
      try {
        ...
        if (resp.data.success) {
          ...
          if (p.starterCode && typeof p.starterCode === "object") {
            setCode(
              p.starterCode[defLang] ||
                `// Starter code for ${defLang} not found.\n`,
            );
          } else {
            setCode(
              p.starterCode || `// Write your ${defLang} solution here.\n`,
            );
          }
        } else {
  ...
  const onLangChange = (e) => {
    ...
    if (problem?.starterCode && typeof problem.starterCode === "object") {
      setCode(
        problem.starterCode[nl] || `// Starter code for ${nl} not available.\n`,
      );
    } else {
  ...
  // ML Complexity Estimation (NO execution)
  const handleEstimateComplexity = async () => {
    if (loadingAuth) {
      alert("Auth still loading");
      return;
    }
    ...
    try {
      ...
      const response = await submissionService.estimateComplexity(payload);

      if (response?.success && response?.data) {
        setMlEstimate(response.data);
      } else {
        throw new Error(response?.message || "Estimation failed");
      }
    } catch (err) {
      console.error("ML Estimate error:", err);
      alert(
        err.response?.data?.message ||
          err.message ||
          "Failed to estimate complexity",
      );
    } finally {
  ...
  return (
    <div className="p-4 md:p-6 max-w-full dark:bg-gray-900 dark:text-gray-100 min-h-screen relative">
      ...
      <div
        className={`grid grid-cols-1 lg:grid-cols-2 gap-6 transition-all duration-300 ${showAIModal ? "blur-md filter brightness-50 pointer-events-none" : ""}`}
      >
        {/* ——— Left: Description ——— */}
        <div className="prose dark:prose-invert max-w-none md:pr-4 lg:overflow-y-auto lg:max-h-[calc(100vh-100px)] custom-scrollbar">
          ...
          <div className="flex items-center gap-x-3 text-sm mb-3">
            <span
              className={`px-2.5 py-0.5 text-xs font-semibold rounded-full border ${getDifficultyColor(problem.difficulty)} capitalize`}
            >
              {problem.difficulty}
            </span>
            <span>
            ...
            <span>
              ...
              {problem.memoryLimit / 1024}MB
            </span>
          </div>
          <h3 className="text-xl font-semibold mt-6 mb-2 border-b pb-1.5 dark:border-gray-700">
            Description
          </h3>
          <div
          ...
          {problem.testCases?.filter((tc) => tc.isSample).length > 0 && (
            <>
              <h3 className="text-xl font-semibold mt-8 mb-3 border-b pb-1.5 dark:border-gray-700">
                Sample Cases
              </h3>
              {problem.testCases
                ...
                .map((tc, i) => (
                  <div
                    key={i}
                    className="my-4 p-4 border rounded dark:border-gray-700 bg-gray-800/50 text-xs"
                  >
                    <p className="font-medium text-gray-300 mb-1">
                      Sample Input {i + 1}:
                    </p>
                    <pre className="bg-gray-900/70 p-2.5 rounded mt-1 mb-2 whitespace-pre-wrap text-gray-200">
                      {tc.input}
                    </pre>
                    <p className="font-medium text-gray-300 mb-1 mt-3">
                      Sample Output {i + 1}:
                    </p>
                    <pre className="bg-gray-900/70 p-2.5 rounded mt-1 whitespace-pre-wrap text-gray-200">
                      {tc.expectedOutput}
                    </pre>
                  </div>
                ))}
        ...
        <div className="flex flex-col lg:max-h-[calc(100vh-100px)]">
          ...
          <div className="flex flex-wrap gap-3 mb-4">
            ...
            <Button
              ...
              disabled={isProcessing || isRequestingAIHelp || isEstimating}
              content={isEstimating ? "Estimating..." : "Estimate Complexity"}
              className="bg-purple-600 hover:bg-purple-700"
              icon={<FontAwesomeIcon icon={faBrain} className="mr-2" />}
            />
          </div>
          {mlEstimate && (
            <div className="mt-4 p-4 border rounded-md bg-purple-900/20 border-purple-700 text-sm">
              <h4 className="text-purple-300 font-semibold mb-2 flex items-center">
                <FontAwesomeIcon icon={faBrain} className="mr-2" />
                AI Complexity Estimate (No Execution)
              </h4>

              <p>
                <span className="font-medium">Time Complexity:</span>{" "}
                <span className="font-semibold">
                  {mlEstimate.time_complexity}
                </span>
              </p>
              ...
              <p>
                ...
                <span className="font-semibold">
                  {mlEstimate.space_complexity}
                </span>
              </p>

              <p>
                <span className="font-medium">TLE Risk:</span>{" "}
                <span className="font-semibold">
                  {Math.round((mlEstimate.tle_risk || 0) * 100)}%
                </span>
              </p>

              {mlEstimate.predicted_runtime_ms != null && (
                <p>
                  <span className="font-medium">Predicted Runtime:</span>{" "}
                  {mlEstimate.predicted_runtime_ms.toFixed(2)} ms
                </p>
              )}

              {mlEstimate.predicted_memory_kb != null && (
                <p>
                  <span className="font-medium">Predicted Memory:</span>{" "}
                  {(mlEstimate.predicted_memory_kb / 1024).toFixed(2)} MB
                </p>
              )}
            </div>
          )}
          ...
          {currentSubmission && (
            <div className="mt-4 p-4 border rounded-md dark:border-gray-700 bg-gray-800 overflow-y-auto custom-scrollbar max-h-[calc(100vh-600px)]">
              <h3 className="text-lg font-semibold mb-2">
                Submission Status:{" "}
                <span className={getResultStyle(currentSubmission.verdict)}>
              ...
              {currentSubmission.testCaseResults?.map((tc, i) => {
                ...
                return (
                  <div
                    key={`tc-${i}`}
                    className={`${getResultStyle(tc.status)} mb-1`}
                  >
                    <p className="text-xs">
                    ...
                    {tc.isSample && tc.status === "Wrong Answer" && (
                      <div className="mt-1 text-xs">
                        <p>
                          Input:{" "}
                          <pre className="inline-block truncate bg-gray-600 p-1 rounded">
                            {tc.input}
                          </pre>
                        </p>
                        <p>
              ...
              {(currentSubmission.executionTime !== undefined ||
                currentSubmission.memoryUsed !== undefined) && (
              ...
              {currentSubmission.verdict === "Accepted" &&
                currentSubmission.submissionType === "submit" && (
