...
import { estimateTimeComplexity, estimateSpaceComplexity } from '../utils/complexityEstimator.js';
...
const executeSingleTestCaseInDocker = async (language, code, stdin, timeLimitSec, memoryLimitKB, submissionIdForLog) => {
    ...
    try {
        ...
        const dockerArgs = [
            ...
            '-v', `${hostDirForMount}:${containerDir}`, dockerImage,
...
const processSubmissionJob = async (job) => {
  ...
  try {
    ...
    if (!submission) { 
      console.error(`Worker: Submission ${submissionId} not found for job ${job.id}.`);
      return; 
    }
  } catch (e) { 
  ...
  try {
    ...
    if (!problem) { 
      console.error(`Worker: Problem ${submission.problemId} not found for submission ${submissionId}.`);
      await Submission.findByIdAndUpdate(submissionId, { verdict: 'Internal System Error', compileOutput: 'Problem details not found.' });
      return; 
    }
  } catch (e) { 
     console.error(`Worker: Error fetching problem ${submission.problemId} for submission ${submissionId}:`, e);
  ...
  await Submission.findByIdAndUpdate(submissionId, { verdict: 'Compiling', testCaseResults: [], compileOutput: null, stderr: null, executionTime: 0, memoryUsed: 0, estimatedTimeComplexity: null, estimatedSpaceComplexity: null });
  ...
  for (let i = 0; i < testCasesToRun.length; i++) {
    ...
    try {
        ...
        if (Array.isArray(parsedInput)) {
            ...
            currentInputSize = Math.max(1, Math.floor(Math.log10(Math.abs(parsedInput)) + 1));
        } else {
    ...
    console.log(`WORKER_TC_INFO (${submissionId}): TC ${i+1} - Status: ${execResult.scriptStatus}, InputSize: ${finalInputSize}, Time: ${execResult.scriptTime}, Memory: ${execResult.scriptMemory}`);
    ...
    switch (execResult.scriptStatus) {
        ...
        case 'TIME_LIMIT_EXCEEDED_EXTERNAL': case 'TIME_LIMIT_EXCEEDED': tcStatus = 'Time Limit Exceeded'; if (overallVerdict === 'Accepted') overallVerdict = 'Time Limit Exceeded'; if (!finalStderrForSubmission) finalStderrForSubmission = execResult.scriptOutput || 'Execution timed out.'; maxTime = Math.max(maxTime, problem.cpuTimeLimit || 2); break;
        case 'MEMORY_LIMIT_EXCEEDED': tcStatus = 'Memory Limit Exceeded'; if (overallVerdict === 'Accepted') overallVerdict = 'Memory Limit Exceeded'; if (!finalStderrForSubmission) finalStderrForSubmission = execResult.scriptOutput || 'Memory limit exceeded.'; break;
        case 'RUNTIME_ERROR': tcStatus = 'Runtime Error'; if (overallVerdict === 'Accepted') overallVerdict = 'Runtime Error'; if (!finalStderrForSubmission) finalStderrForSubmission = execResult.scriptOutput; break;
        case 'EXECUTED_SUCCESSFULLY':
            tcActualOutput = execResult.scriptOutput; const userOutputTrimmed = (tcActualOutput || '').trim().replace(/\r\n/g, '\n'); const expectedOutputTrimmed = (tc.expectedOutput || '').trim().replace(/\r\n/g, '\n');
            if (userOutputTrimmed === expectedOutputTrimmed) tcStatus = 'Accepted';
            else { tcStatus = 'Wrong Answer'; if (overallVerdict === 'Accepted') overallVerdict = 'Wrong Answer'; if (!finalStderrForSubmission) finalStderrForSubmission = `Output Mismatch on Test Case ${i + 1}:\nExpected:\n'${expectedOutputTrimmed}'\nGot:\n'${userOutputTrimmed}'`;}
            break;
        default: tcStatus = 'Internal System Error'; if (overallVerdict === 'Accepted') overallVerdict = 'Internal System Error'; if (!finalStderrForSubmission) finalStderrForSubmission = `Unknown script status: ${execResult.scriptStatus}. Output: ${execResult.scriptOutput}`;
    }
    
    resultsForDB.push({ 
        input: tc.input, expectedOutput: tc.expectedOutput, actualOutput: tcActualOutput, 
        status: tcStatus, time: execResult.scriptTime || 0, memory: execResult.scriptMemory || 0, 
        inputSize: finalInputSize,
        isSample: tc.isSample 
    });
    if (overallVerdict === 'Compilation Error' || (submission.submissionType === 'submit' && overallVerdict !== 'Accepted')) break;
  }

  if (submission.submissionType === 'run' && overallVerdict === 'Accepted' && !finalCompileOutput) {
    overallVerdict = 'Partial - Sample Run';
  }

  let estimatedTC = null;
  let estimatedSC = null;
  if (submission.submissionType === 'submit' && overallVerdict === 'Accepted' && resultsForDB && resultsForDB.length >= 3) {
    try {
        estimatedTC = estimateTimeComplexity(resultsForDB);
        estimatedSC = estimateSpaceComplexity(resultsForDB);
        console.log(`WORKER_COMPLEXITY (${submissionId}): Estimated TC: ${estimatedTC}, Estimated SC: ${estimatedSC}`);
 ...
 // --- CONTEST SCORING LOGIC ---
  ...
  const finalUpdateData = {
    ...
    ...( (finalStderrForSubmission && overallVerdict !== 'Accepted' && overallVerdict !== 'Partial - Sample Run' && overallVerdict !== 'Wrong Answer') && {stderr: finalStderrForSubmission})
  };
