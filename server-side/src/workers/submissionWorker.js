        ...
        return new Promise((resolve) => {
            ...
            dockerProcess.on('close', (code, signal) => {
                ...
                if (signal === 'SIGTERM') resolve({ scriptStatus: 'TIME_LIMIT_EXCEEDED_EXTERNAL', scriptTime: timeLimitSec, scriptMemory: 0, scriptOutput: 'Execution surpassed external watchdog timeout.', dockerRawStderr: rawStderr });
                    ...
                    resolve({ scriptStatus: lines[0]?.trim() || 'UNKNOWN_SCRIPT_OUTPUT', scriptTime: parseFloat(lines[1]?.trim()) || 0, scriptMemory: parseInt(lines[2]?.trim(), 10) || 0, scriptOutput: lines.slice(3).join('\n'), dockerRawStderr: rawStderr });
                }
            });
        });
...
const processSubmissionJob = async (job) => {
  ...
  for (let i = 0; i < testCasesToRun.length; i++) {
    ...
    resultsForDB.push({ input: tc.input, expectedOutput: tc.expectedOutput, actualOutput: tcActualOutput, status: tcStatus, time: execResult.scriptTime || 0, memory: execResult.scriptMemory || 0, isSample: tc.isSample });
    if (overallVerdict === 'Compilation Error' || (submission.submissionType === 'submit' && overallVerdict !== 'Accepted')) break;
  }
  if (submission.submissionType === 'run' && overallVerdict === 'Accepted' && !finalCompileOutput) overallVerdict = 'Partial - Sample Run';
  await Submission.findByIdAndUpdate(submissionId, { verdict: overallVerdict, testCaseResults: resultsForDB, compileOutput: finalCompileOutput, executionTime: maxTime, memoryUsed: maxMemory, ...((finalStderrForSubmission && overallVerdict !== 'Accepted' && overallVerdict !== 'Partial - Sample Run' && overallVerdict !== 'Wrong Answer') && {stderr: finalStderrForSubmission})});
